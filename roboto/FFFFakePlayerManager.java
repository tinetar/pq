...
public enum FFFFakePlayerManager {
    ...
    private static final long AFK_THRESHOLD = 60 * 1000L; 
    ...
    public void cleanGhosts() {
        ...
        public void teleportDeadBot(FFFFakePlayer bot) {
    	        String filepath = bot.isGmSpawned()
    	            ? "data/fakebots/gm-metato8anato-5spor-ffff.txt"
    	            : "data/fakebots/list-metato8anato-5spor-ffff.txt";
    	
    	        int offsetRange = bot.isGmSpawned() ? 200 : 500;
    	
    	        List<Location> baseSpots = FFFFakeHelpers.loadSpawnLocations(filepath);
    	        if (baseSpots == null || baseSpots.isEmpty()) {
    	            _log.warning("No death respawn spots found in file: " + filepath);
    	            return;
    	        }
    	
    	        Location chosenBase = baseSpots.get(Rnd.get(baseSpots.size()));
    	        Location finalLoc = FFFFakeHelpers.getOffsetLocation(chosenBase, offsetRange);
    	
    	        bot.teleToLocation(finalLoc, 0); // ✅ 0 είναι το heading

    	        _log.info("Bot " + bot.getName() + " teleported on death to " + finalLoc + " (gm=" + bot.isGmSpawned() + ")");
    	    }
    ...
    public List<FFFFakePlayer> getFFFFakePlayers() {
        ...
        public void verifyBotIntegrity() {
        	        int removed = 0;
                   ...
                   Player bot = it.next();
        	           ...
        	           boolean isGhost = !bot.isOnline() || bot.isTeleporting();
        	            boolean isClanGhost = (bot.getClan() == null);
        	            boolean isUnderground = bot.getZ() < 0;
        	
        	            long lastMove = (bot instanceof FFFFakePlayer)
        	                ? ((FFFFakePlayer) bot).getLastMoveTimestamp()
        	               : now;
        	            boolean isAfk = (now - lastMove) > AFK_THRESHOLD;
        	
        	            if (isGhost || isClanGhost || isUnderground || isAfk) {
        	               _log.info("verifyBotIntegrity: Removing bot [" + bot.getName() + "]"
        	                    + " (online=" + bot.isOnline()
        	                   + ", teleporting=" + bot.isTeleporting()
        	                    + ", clan=" + (bot.getClan() != null ? bot.getClan().getName() : "null")
        	                    + ", Z=" + bot.getZ()
        	                    + ", AFK=" + isAfk + ")");
        	                it.remove();
        	        ...
        	        if (removed > 0)
        	           _log.info("verifyBotIntegrity: Removed " + removed + " bot(s) (ghost/clan/underground/afk).");
