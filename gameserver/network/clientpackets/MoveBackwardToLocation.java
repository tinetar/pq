...
public class MoveBackwardToLocation extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(activeChar.isCControllingFFakePlayer()) 
		{
			...
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
			ffakePlayer.getAI().setIntention(CtrlIntention.MOVE_TO, new Location(_targetX, _targetY, _targetZ));
			return;
		}
		...
		if(activeChar.isCCControllingFFFakePlayer()) 
		{
			...
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
			fffakePlayer.getAI().setIntention(CtrlIntention.MOVE_TO, new Location(_targetX, _targetY, _targetZ));
			return;
		}	
		...
		if(activeChar.isCCCControllingFFFFakePlayer()) 
		{
			...
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
			ffffakePlayer.getAI().setIntention(CtrlIntention.MOVE_TO, new Location(_targetX, _targetY, _targetZ));
			return;
		}
		...
		if(activeChar.isCCCCControllingFFFFFakePlayer()) 
		{
			...
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
			fffffakePlayer.getAI().setIntention(CtrlIntention.MOVE_TO, new Location(_targetX, _targetY, _targetZ));
			return;
		}
