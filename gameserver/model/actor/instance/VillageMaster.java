	...
	public static final void showPledgeSkillList(Player player)
	{
		...
		if (skills.isEmpty())
		{
			if (player.getClan().getLevel() < 8)
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.DO_NOT_HAVE_FURTHER_SKILLS_TO_LEARN_S1).addNumber(Math.max(player.getClan().getLevel() + 1, 5)));
			else
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(0);
