...
public class DerbyTrackManagerNpc extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.startsWith("BuyTicket"))
		{
			...
			if (val < 10)
			{
				...
				for (int i = 0; i < 8; i++)
				{
					int n = i + 1;
					search = "Mob" + n;
					html.replace(search, DerbyTrackManager.getInstance().getMonsters()[i].getTemplate().getName());
				}
				search = "No1";
				if (val == 0)
					html.replace(search, "");
			...
			else if (val < 20)
			{
				...
				search = "Mob1";
				replace = DerbyTrackManager.getInstance().getMonsters()[player.getRace(0) - 1].getTemplate().getName();
		...
		else if (command.equals("ShowOdds"))
		{
			...
			for (int i = 0; i < 8; i++)
			{
				...
				html.replace("Mob" + n, DerbyTrackManager.getInstance().getMonsters()[i].getTemplate().getName());
		...
		else if (command.startsWith("ShowTicket"))
		{
			...
			final HistoryInfo info = DerbyTrackManager.getInstance().getHistory().get(raceId - 1);
			if (info == null)
			{
				super.onBypassFeedback(player, "Chat 0");
				return;
		...
		else if (command.equals("ViewHistory"))
		{
			...
			final List<HistoryInfo> history = DerbyTrackManager.getInstance().getHistory();
	...
	public void removeKnownObject(WorldObject object)
	{
		...
		if (object instanceof Player)
		{
			...
			for (Npc npc : DerbyTrackManager.getInstance().getMonsters())
