...
public class Pet extends Summon
{
	...
	private static final String STORE_PET = "INSERT INTO pets (name,level,curHp,curMp,exp,sp,fed,item_obj_id) VALUES (?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE name=?,level=?,curHp=?,curMp=?,exp=?,sp=?,fed=?";
	...
	public void store()
	{
			...
			PreparedStatement ps = con.prepareStatement(STORE_PET))
		{
			...
			ps.setInt(10, getStat().getLevel());
			ps.setDouble(11, getStatus().getCurrentHp());
			ps.setDouble(12, getStatus().getCurrentMp());
			ps.setLong(13, getStat().getExp());
			ps.setInt(14, getStat().getSp());
			ps.setInt(15, getCurrentFed());
			ps.executeUpdate();
		}
		catch (Exception e)
		{
